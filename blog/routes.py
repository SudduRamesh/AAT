...
def question():
  form =  QuestionForm()
  questions = Question.query.join(User, Question.initiator == User.id).with_entities(Question.id,Question.question, Question.category,Question.module_code, Question.created_date,\
                                                                                     Question.modify_date,Question.answers, Question.level, Question.points, User.first_name,\
                                                                                     User.last_name).order_by(Question.created_date).all()
                                                                    
  return render_template('question.html',title='Questions', current_user=current_user, form=form, questions=questions)
    ...
    if checkmark:
      ...
      if form.validate_on_submit():
        try:
          ...
          newQuestion = Question(module_code=form.module_code.data, question=form.question.data, answers=result_dict, initiator=int(current_user.id),category=form.category.data) 
...
def logout():
  try:
    logout_user()
    flash('Logout Succsuful','success')
    return redirect(url_for('login'))
  except:
    flash('Error','danger')
...
def forget():
  ...
  if request.method == 'POST':
    ...
    if not reset_user:
      flash('This email does not exist, please double check')
      return redirect(url_for('login'))
    else:
      send_reset_pw_mail(reset_user)
      flash('Reset password email has been sent, please check your mail')
      return redirect(url_for('home'))
...
def reset(token):
  ...
  if request.method == 'POST':
    if reset_form.password.data != reset_form.confirm_password.data:
      flash('Your confirm password does not match your password. please re-enter.')
  ...
  if option == "user":
    if current_user.is_authenticated and current_user.access_type == 'admin' or  current_user.access_type == 'developer':
        ...
        try:
          ...
          print("Error, Couldnt delete the account")
        ...
        try:
          ...
          print("Error, Couldnt delete the question")
